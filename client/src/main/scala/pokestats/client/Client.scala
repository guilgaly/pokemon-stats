...
object UrlPartAdapters {
  implicit val pokemonDetailsUrlPart: UrlPart[PokemonSummary] =
    new UrlPart[PokemonSummary] {
      ...
      override def fromParts(parts: List[String])(
        ...
        case pokeId :: Nil =>
          Future.successful(PokemonSummary(pokeId.toInt, ""))
        case _ => Future.failed(new IllegalArgumentException)
      }
    }
}

object Client extends routerx.implicits.Defaults {

  private implicit val owner: Ctx.Owner = rx.Ctx.Owner.safe()
